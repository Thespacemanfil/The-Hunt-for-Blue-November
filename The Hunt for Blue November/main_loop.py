...
class ship:
    ...
    def ai(ship, player, enemy_torpedoes):
        ...
        if random.randint(1,75) == 1:
            ship.tgt_speed = random.randint(30,100)
...
class torpedo:
    ...
    def threeD_movement(torpedo, torpedoes, enemies, dt):
        ...
        # Calculate bearing (horizontal direction) - angle in the x-z plane
        enemy_heading = math.atan2(dx, dz) % (2 * np.pi) # negative dz because y-axis is positive in the "up" direction
        tgt_pitch = math.atan2(dy, distance_2d)

        #bearing difference
        bearing_diff = min(abs(enemy_heading - torpedo.heading) % (2 * math.pi), (2 * math.pi) - abs(enemy_heading - torpedo.heading) % (2 * math.pi))

        #calculate lead
        if False:
            torpedo.tgt_heading = data.proportional_navigation(torpedo.speed, torpedo.memory, torpedo.heading, enemy_heading, dt)
        else:
            torpedo.tgt_heading = enemy_heading
...
def initialise():
    ...
    x = random.randint(2000,7000) * random.choice([-1, 1])
    y = random.randint(2000,7000) * random.choice([-1, 1])
