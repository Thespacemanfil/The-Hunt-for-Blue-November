...
class submarine:
    ...
    def torpedo(submarine,torpedoes,target):
        ...
        torpedoes.append(torpedo("hyena",submarine.x,submarine.y,submarine.z,submarine.speed,submarine.heading,submarine.heading,0,150,target,10,1,9999,1000,2,2,2))
...
class torpedo:
    ...
    def threeD_movement(torpedo, dt):
...
def initialise():
    ...
    player = submarine("narwhal", "SSN", 0, 0, 0, 0, 3.14, 0, 0, 0, enemies[0], 10, 0.5, 100, 500, 20, 2, 0.3, 0.3)
...
def gameloop(FPS, dt, map_scale, sensitivity, player, enemies, torpedoes):
    ...
    while True:
        ...
        for ship in enemies:
            ...
            if i == player.target: graphics.draw_selection(map_scale, player, ship)
        ...
        if keys[K_t]:
            pass
        if keys[K_f]:
            if reload <= 0 and player.torpedo_load > 0: player.torpedo(torpedoes,player.target); reload = 5; player.torpedo_load -=1
            else: reload -= dt
        if keys[K_m]:
