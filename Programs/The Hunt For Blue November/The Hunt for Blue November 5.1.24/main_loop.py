...
class ship:
    ...
    def torpedo(ship,enemies,torpedoes,target):
        #torpedoes.append(torpedo("hyena",submarine.x,submarine.y,submarine.z,submarine.speed,submarine.heading,submarine.heading,0,150,target,10,50,9999,1000,2,2,2))
        torpedoes.append(torpedo("crabb",ship.x,ship.y,ship.z,ship.speed + 50,ship.heading,ship.heading,0,150,enemies[ship.target],10,0.5,9999,1000,2,2,2))
...
class torpedo:
    ...
    def threeD_movement(torpedo, torpedoes, player, enemies, dt):
        targets = enemies
        if torpedo.target in targets: target = torpedo.target; print("target found")
        ...
        if np.hypot(distance_2d, dy) < 50:
            try:
                print("detonate")
                if data.kill(enemies, player, torpedo.target) == True: gameover()
                print("Vessel sunk!")
...
def initialise():
    ...
    for i in range(random.randint(1,5)):
        enemies.append(ship("krankenhaus", "DDG", random.randint(-2000,2000), 0, random.randint(-2000,2000), 0, 200, random.randint(10,100), random.uniform(0,3.14), 0, 2, 5, 200, 100, 5, 7, 4, 10))

    player = submarine("narwhal", "SSN", 0, 0, 0, 0, 0, 0, 0, 0, 0, 10, 0.2, 100, 500, 20, 2, 0.3, 0.3)

    graphics.draw_map()

    gameloop(FPS, dt, map_scale, sensitivity, player, enemies, torpedoes)

def gameloop(FPS, dt, map_scale, sensitivity, player, enemies, torpedoes):
    rad = 2 * np.pi
    reload = 0
    cooldown = 0
    endgame = 5
    ...
    while True:
        ...
        for torpedo in torpedoes:
            torpedo.threeD_movement(torpedoes, player, enemies, dt)
